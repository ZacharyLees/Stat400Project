...
Continuing on with our research, we created a graph that shows the distribution of Gender  (male or female) to observe the split between genders collected and reported credit fraud.
```{r}
...
```
Based upon the graph above we can say that about twice the amount of clients reported to be females verses males had no reported credit fraud. However, from further observation, we can see that females have a slightly higher count than males for reported credit fraud. Continuing forward in our data analysis we will observe other realtionships between the variables.
...
```
Looking at the data comparing total income and the amount of credit fraud reported by gender, we can see that females typically show a pattern to have more reported credit fraud than the males do with the same to similar total incomes. Looking at this graph we have to keep in mind that more females than males were also surveyed, so this graph alone is not enough to provide enough evidence to support our research statement.
