...
=======
```{r}
#his example from lecture 20
model3 <- lmer(na ~ orch + large + orch:large + (1 + large | id),
               REML = FALSE,
               data = music)
summary(model3)

model <- lmer(reaction ~ meaning + old + meaning:old + (1 + old | id),
               REML = FALSE,
               data = music)
summary(model3)
```
...
## Example 4
```{r}
model3.reduced <- lmer(na ~ orch + large + orch:large + (1 | id),
               REML = FALSE,
               data = music)
...
>>>>>>> 2f8414fcb66378a42193a38f63c1088c19099db2
